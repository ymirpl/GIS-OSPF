...
/**
 * Klasa wyswietlajaca siec za pomoca bibliteki JUNG.
 */
public class PrettyGraph extends JApplet {
	...
	/**
	 * Metoda przepisuje graf wyznaczony przez algorytm na graf uzywany przez
	 * JUNGa
	 */
	public void getGraph(edu.pw.gis.graph.Graph g) {
		...
		for (edu.pw.gis.graph.Edge e : g.edgeList) {
			...
			if (g.edgeList.get(e.id).flowSum > 0) // wyswietlane sa jedynie krawedzie, przez ktore jest jakis przeplyw
				jungGraph.addEdge(e.id, e.source.name, e.target.name,
						EdgeType.DIRECTED);
		}
	}
	...
	/**
	 * Rysowanie grafu
	 * @param g struktura stworzonego przez nas grafu
	 */
	public PrettyGraph(edu.pw.gis.graph.Graph g) {
		...
		Layout<String, Integer> layout = new FRLayout<String, Integer>(
				jungGraph);
		...
		/**
		 * Opis krawedzi wyswietlany na ekranie
		 */
		Transformer<Integer, String> stringer = new Transformer<Integer, String>() {
			DecimalFormat twoDigit = new DecimalFormat("#,##0.00");

			public String transform(Integer e) {
				return "" + twoDigit.format(myGraph.edgeList.get(e).usage)
						+ " / " + myGraph.edgeList.get(e).weight + " / "
						+ twoDigit.format(myGraph.edgeList.get(e).flowSum)
						+ " / " + myGraph.edgeList.get(e).capacity;
			}
		};
		vv.getRenderContext().setEdgeLabelTransformer(stringer);
		vv.getRenderContext().setEdgeDrawPaintTransformer(
				new PickableEdgePaintTransformer<Integer>(vv
